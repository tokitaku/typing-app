...
│   │   ├── application/    # 複数 feature で共有する DTO
│   │   ├── domain/         # 共有する型定義
│   │   ├── shared/         # 明確に再利用される API client / browser helper
...
- `frontend/src/application` には DTO、`frontend/src/shared` には複数 feature で再利用する API client と browser helper だけを置く
